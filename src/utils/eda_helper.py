...
def agg_numerical_features(df, key, numerical_features):
    ...
    for col in numerical_features:
        agg_func = [
            function(col).alias(f"{col}_{func_name}")
            if func_name != "median"
            else function.alias(f"{col}_{func_name}")
            for func_name, function in funcs.items()
        ]
...
def toPandas(spark_df, pkey=None):
    ...
    if pkey:
        pd_df["distinct_cnt_{pkey}"] = pd_df[pkey].nunique(
            dropna=False
        )  # by default dropna is set to True
        pd_df["records"] = pd_df.shape[0]
    return pd_df


def get_cumsum(frequency_table, count_col):
    frequency_table["perc"] = (
        frequency_table[count_col] / frequency_table[count_col].sum()
    )
...
def rename_columns(include_cols, prefix="sa_dly_"):
    renamed_columns = ["party_id", "as_of_dt"] + [
        "sa_dly_" + col
        for col in include_cols
        if not (col == "party_id" or col == "as_of_dt")
    ]
...
class ExcelWriter:
    ...
    def write_to_excel(self, sheet_name, df):
        with pd.ExcelWriter(
            f"{self.base_folder}/{self.filename}.xlsx",
            mode="a",
            engine="openpyxl",
...
def gen_label_report(df_pd, writer, cust_key, label, date_col):
    ...
    stat_total_week = pd.pivot_table(
        df_pd,
        values=cust_key,
        index=[label],
        columns=["week"],
    ...
    stat_total_month = pd.pivot_table(
        df_pd,
        values=cust_key,
        index=[label],
        columns=["month"],
...
def get_data_summary(df):
    ...
    for col in tqdm.tqdm(df.columns):
        ...
        proposed_data_type = (
            "numerical"
            if decimal.Decimal in dtype
            or int in dtype
            or (float in dtype and str not in dtype)
            else "categorical"
        )
    ...
    # Add summary
    summary["remarks"] = ""
    summary["remarks"] = summary.apply(
        lambda x: f"{x['remarks']}, missing_rate==1"
        if x["missing_rate"] == 1
        else x["remarks"],
        axis=1,
    )
    summary["remarks"] = summary.apply(
        lambda x: f"{x['remarks']}, zero_rate==1"
        if x["zero_rate"] == 1
        else x["remarks"],
        axis=1,
    )
    summary["remarks"] = summary.aply(
        lambda x: f"{x['remarks']}, cardinality==1"
        if x["num_unique"] == 1
        else x["remarks"],
        axis=1,
    )
...
def get_stat(pd_df, col, label_col, label, data_summary):
    ...
    stat = (
        pd_df.groupby([col])["pseudo_label"]
        .value_counts(dropna=False)
        .unstack()
        .reset_index()
        .rename(columns={0: "neg", 1: "pos", col: "bin"})
        .fillna(0)
    )
    stat["bin"] = stat["bin"].apply(str)

    if "(" in stat["bin"].iloc[0] and "]" in stat["bin"].iloc[0]:
        stat["bin2"] = stat["bin"].apply(
            lambda x: float(x.split(",")[0][1:]) if x != "nan" else 999999999
        )
    ...
    stat[f"iv"] = (stat["neg_rate"] - stat["pos_rate"]) * np.log(
        (stat["neg_rate"] + eps) / (stat["pos_rate"] + eps)
    )
    iv = stat["iv"], sum()
    stat["proposed_data_type"] = data_summary.loc[
        data_summary["var"] == col, "proposed_data_type"
    ].iloc[0]
    stat["label"] = label
...
def get_stat_verbose(pd_df, col, label_col, label, data_summary):
    ...
    columns_order = ["label", "var", "bin", "proposed_data_type"] + [
        col
        for col in stat.columns
        if col not in ["label", "var", "bin", "proposed_data_type"]
    ]
...
def get_stat_main(df, data_summary):
    numerical_features = get_numerical_features(
        df
    )  # remember to exclude primary key columns
    ...
    for lab in tqdm.tqdm(df["next_7_day_labels"].unique()):
        ...
        for col in numerical_features:
            if (
                decimal.Decimal
                in data_summary.loc[data_summary["var"] == col]["type"].iloc[0]
            ):
                df_copy[col] = df_copy[col].apply(
                    lambda x: float(x) if (str(x) != "np.nan" and x != None) else x
                )
            if data_summary.loc[data_summary["var"] == col, "num_unique"].iloc[0] > 10:
                df_copy[col] = pd.qcut(df_copy[col], q=10, duplicates="drop").astype(
                    str
                )
            else:
                df_copy[col] = df_copy[col].fillna(9999999)  # fill missing with 9999999
            num_stat_verbose = get_stat_verbose(
                df_copy, col, "next_7_day_labels", lab, data_summary=data_summary
            )
        ...
        for col in categorical_features:
            ...
            df_copy[col] = df_copy[col].apply(
                lambda x: x if str(x) != "np.nan" and str(x) != None else "NA"
            )
            cat_stat_verbose = get_stat_verbose(
                df_copy, col, "next_7_day_labels", lab, data_summary=data_summary
            )
...
def stat_feature_analysis(sample_pd, all_stat, all_stat_verbose):
    ...
    non_sigfig_labels = label_vc[
        label_vc < 100
    ].index.tolist()  # labels that are non-significant
    all_stat_copy = all_stat.copy()
    all_stat_copy.loc[
        all_stat_copy["label"].apply(lambda x: x in non_sigfig_labels), "iv"
    ] = 0

    agg_stat = all_stat.groupby("var")[
        "iv"
    ].describe()  # count, mean, std, min, 25% .... max
    ...
    agg_stat2 = (
        all_stat_copy[~all_stat_copy["label"].apply(lambda x: x in non_sigfig_labels)]
        .groupby("var")["iv"]
        .describe()
    )
    agg_stat2.columns = [f"iv_{col}" for col in agg_stat2]
    agg_stat2 = agg_stat2.reset_index().rename(
        columns={"iv_count": "num_unique_labels"}
    )
...
class SampledDataset:
    ...
    def generate_samples_labels(self, modelling_data):
        ...
        modelling_data = modelling_data.filter(
            F.col("date_col").between(start_date, end_date)
        )  # dataset containing positive interactions (cin, date, label)
        # Hive timestamp is interpreted as UNIX timestamp in seconds
        days = lambda i: i * 86400
        Window = lambda x: x
        look_ahead_window = (
            Window.partitionBy("user_id")
            .orderBy(F.col("unix_date"))
            .rangeBetween(0, days(1 + 7))
        )
        look_ahead_gap_window = Window.partitionBy("user_id").orderBy(
            F.col("unix_date").rangeBetween(days(3), days(3 + 7))
        )

        modelling_data = modelling_data.withColumn(
            "next_7_day_labels", F.collect_set("intent").over(look_ahead_window)
        )
        modelling_data = modelling_data.withColumn(
            "next_7_day_labels_with_gap",
            F.collect_set("intent").over(look_ahead_gap_window),
        )
        label_data = modelling_data.select(
            "user_id",
            "date",
            "next_7_day_labels",
            F.explode("next_7_day_labels").alias("label"),
        ).drop_duplicates()
        sampling = True
        spark = lambda x: x
        if sampling == True:
            ...
            label_data_with_pkeys = label_data.join(
                cifs, label_data["user_id"] == cifs["kais"], "inner"
            ).persist()
            # it is possible for some customers to not be in the master table.
            label_data_with_pkeys_final = label_data_with_pkeys.filter(
                F.col("cif_id").isNotNull()
            )
            ...
            for col, dtype in label_data_with_pkeys_final.dtypes:
                if "date" in dtype or "time" in dtype:
                    label_data_with_pkeys_final = (
                        label_data_with_pkeys_final.withColumn(
    ...
    def load_demo_sample(self, demo):
        ...
        demo = demo.filter(F.col("as_of_dt").between(start_date, end_date)).select(
            *oc_cols
        )
        ...
        Window = lambda x: x
        lookback_window = (
            Window.partitionBy("cif_id")
            .orderBy("date")
            .rowsBetween(Window.unboundedPreceding, 0)
        )
        for col in demo.columns:
            if col not in ["cif_id", "date"]:
                demo_ft = demo_ft.withColumn(
                    col, F.last(F.col(col), ignore_nulls=True).over(lookback_window)
                )
        return
    ...
    def load_historical_interactions(self, cb_int):
        ...
        raw_chatbot = (
            cb_int.filter(F.col("businessdate") >= start_date)
            .filter(F.col("businessdate") < end_date)
            .select(*columns_to_use)
        )
        raw_chatbot = raw_chatbot.withColumn(
            "date", F.to_date(F.to_timestamp(F.col("request_timestamp")))
        )
        raw_chatbot = raw_chatbot.withColumn(
            "unix_date", F.unix_timestamp(F.col("date"))
        )
        ...
        w = (
            Window.partitionBy("user_id")
            .orderBy("unix_date")
            .rangeBetween(-days(60), -days(1))
        )
        raw_chatbot = raw_chatbot.withColumn(
            "historical_interactions", F.collect_list(F.col("intent").over(w))
        )

        chatbot_columns = raw_chatbot.columns
        raw_chatbot = raw_chatbot.select(
            *chatbot_columns,
            F.explode_outer("historical_interactions").alias("cb_past_intent"),
        )

        # Ensure the wide transformation contains all the values listed in pivot_list
        pivot_list = ["val1", "val2"] + ["null"]
        chatbot = (
            raw_chatbot.groupy("user_id", "date")
            .pivot("past_cb_intent", pivot_list)
            .agg(F.count(F.col("user_id")))
            .fillna(0)
        )

        labels = self.generate_samples_labels(modelling_data=None)  # load from parquet
        raw_chatbot = labels.join(
            chatbot, ["user_id"], ["date"]
        )  # inner join, because we just store the dates that have chatbot features
...
class DataProfiler:
    ...
    def analyze_numerical_features(self, df, num_cols, profile_df):
        ...
        for col in num_cols:
            ...
            profile_df.loc[profile_df.shape[0], :] = [col] + list(
                metadata_dict[col].values()
            )
        return profile_df
    ...
    def analyze_categorical_features(self, df, num_cols, profile_df):
        ...
        for col in num_cols:
            ...
            # get range of data
            metadata_dict[col]["range_of_data"] = (
                "not_applicable"
                if not self.is_date_field(df, col)
    ...
    def profile_data_sample(
        ...
        if partition_col and partition_start_date and partition_end_date:
            ...
            df_sample = df.filter(
                F.col(partition_col).between(partition_start_date, partition_end_date)
            ).toPandas()
            df_profile = self.get_data_profile(df_sample.limit(sample_size))
            min_date = str(
                df.select(F.min(F.col(partition_col)).alias("min_dt")).take(2)[0].min_dt
            )
            max_date = str(
                df.select(F.max(F.col(partition_col)).alias("max_dt")).take(2)[0].max_dt
            )
...
class FeatureAnalysis:
    ...
    def get_feature_cols(self, df_pd):
        if self.verbose:
            print(
                "Selecting the input features that are not (1) primary keys (2) date keys or (3) the label"
            )
        self.features = [
            col
            for col in df_pd.columns
            if col not in self.pkeys + self.dkeys + [self.label]
        ]
        return self.features

    def get_valid_features_from_summary(
        self, summary, df_pd, original_features, label, verbose=True
    ):
        if self.verbose:
            print(
                "Selecting the feasture that have (1) Missing rate != 1 (2) zero_rate != 1 and (3) Cardinality != 1"
            )
        self.all_missing_columns = (
            summary[summary["missing_rate"] == 1]["var"].unique().tolist()
        )
        self.all_zero_columns = (
            summary[summary["zero_rate"] == 1]["var"].unique().tolist()
        )
        self.all_single_cardinality_columns = (
            summary[summary["num_unique"] == 1]["var"].unique().tolist()
        )

        data_summary_drop_cols = list(
            set(
                self.all_missing_columns
                + self.all_zero_columns
                + self.all_single_cardinality_columns
        ...
        df_pd = df_pd[
            [
                col
                for col in df_pd[original_features + [label]].columns
                if col not in data_summary_drop_cols
            ]
        ]
    ...
    def get_categorical_features_from_summary(self, feature_summary):
        ...
        categorical_columns = (
            feature_summary[feature_summary["proposed_data_type"] == "categorical"][
                "var"
            ]
            .unique()
            .tolist()
    ...
    def get_numerical_features_from_summary(self, feature_summary):
        ...
        numerical_columns = (
            feature_summary[feature_summary["proposed_data_type"] == "numerical"]["var"]
            .unique()
            .tolist()
        )
    ...
    def fill_missing_numerical_columns(self, df_pd, numerical_columns, fill_value=0):
        if self.verbose:
            print(
                "Filling missing values in numeircal_column with value : {fill_value}"
            )
        df_pd = df_pd.copy()
    ...
    def fill_missing_categorical_columns(
        self, df_pd, categorical_columns, fill_value="NAN"
    ):
        if self.verbose:
            print(
                "Filling missing values in categorical_columns with value: {fill_value}"
            )
        df_pd = df_pd.copy()
    ...
    def cast_decimals_from_summary(self, df_pd, feature_summary, numerical_columns):
        ...
        for col in tqdm.tqdm(numerical_columns):
            if (
                decimal.Decimal
                in feature_summary.loc[feature_summary["var"] == col]["type"].iloc[0]
            ):
    ...
    def fill_missing_labels(self, y_train, y_test):
        missing_labels_in_y_train = [
            col for col in y_test.unique() if col not in y_train.unique()
        ]
        missing_labels_in_y_test = [
            col for col in y_train.unique() if col not in y_test.unique()
        ]
    ...
    def get_value_counts(self, y):
        ...
        y_dist_perc = (
            ...
            .reset_index()
            .rename(columns={"index": label_name, label_name: "percentage"})
        )
        y_dist_cnt = (
            y.value_counts(normalize=False)
            .reset_index()
            .rename(columns={"index": label_name, label_name: "count"})
        )
    ...
    def calculate_distribution(self, y, y_train, y_test):
        ...
        y_train_dist = (
            self.get_value_counts(y_train)
            .set_index(label_name)
            .add_suffix("_train")
            .reset_index()
        )
        y_test_dist = (
            self.get_value_counts(y_test)
            .set_index(label_name)
            .add_suffix("_test")
            .reset_index()
        )

        all_dist = y_dist.merge(y_train_dist, on=[label_name], how="left").merge(
            y_test_dist, on=[label_name], how="left"
        )
    ...
    def get_train_test_counts(self, X, X_train_df, X_test_df, cust_key):
        ...
        unique_counts = [
            X[cust_key].unique(),
            X_train_df[cust_key].unique(),
            X_test_df[cust_key].unique(),
        ]
        df = pd.DataFrame(
            {
    ...
    def create_train_test_split(
        self, df_pd, cust_key, label, test_size=0.2, random_state=33
    ):
        if self.verbose:
        ...
        X_train_cin, X_test_cin = train_test_split(
            X_cin, test_size=test_size, random_state=random_state
        )
    ...
    def run_data_pipeline(self, df_spark):
        ...
        features = self.get_valid_features_from_summary(
            summary=summary, df_pd=df_pd, original_features=features, label=self.label
        )
        self.feature_summary = summary[summary["var"].apply(lambda x: x in features)]
        self.categorical_columns = self.get_categorical_features_from_summary(
            self.feature_summary
        )
        self.numerical_columns = self.get_numerical_features_from_summary(
            self.feature_summary
        )

        df_pd = self.fill_missing_numerical_columns(
            df_pd, self.numerical_columns, fill_value=self.numerical_missing_value
        )
        df_pd = self.fill_missing_categorical_coluns(
            df_pd, self.categorical_columns, fill_value=self.categorical_missing_value
        )
        df_pd = self.cast_decimals_from_summary(
            df_pd, self.feature_summary, self.numerical_columns
        )
    ...
    def run_model_pipeline(self, df_pd, **kwargs):
        ...
        X_train, X_test, y_train, y_test = self.create_train_test_split(
            df_pd, self.cust_key, self.label, test_size=0.2, random_state=33
        )
    ...
    def gen_feature_report(self, number_of_features):
        ...
        feature_importance_df["is_model_selected_flag"] = feature_importance_df[
            "index"
        ].apply(lambda x: 1 if x < number_of_features else 0)
        feature_importance_df = feature_importance_df[
            ["feature", "gain_importance", "split_importance", "is_model_selected_flag"]
        ]

        original_features = self.summary[["var"]].copy()
        feature_selection_report = original_features.merge(
            feature_importance_df, left_on="var", right_on="feature", how="left"
        )
        feature_selection_report["used_as_model_input"] = feature_selection_report[
            "gain_importance"
        ].apply(lambda x: 1 if str(x) != "nan" else 0)
        feature_selection_report["remarks"] = ""
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"{x['remarks']}, missing_rate==1"
            if x["var"] in self.all_missing_columns
            else x["remarks"],
            axis=1,
        )
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"{x['remarks']}, zero_rate==1"
            if x["var"] in self.all_zero_columns
            else x["remarks"],
            axis=1,
        )
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"{x['remarks']}, cardinality==1"
            if x["var"] in self.all_single_cardinality_columns
            else x["remarks"],
            axis=1,
        )
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"{x['remarks']}, is model selected"
            if x["is_model_selected_flag"] == 1
            else f"{x['remarks']}, is not model selected"
            if x["is_model_selected_flag"] == 0
            else x["remarks"],
            axis=1,
        )
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"{x['remarks']}, is primary key"
            if x["var"] in self.pkeys
            else x["remarks"],
            axis=1,
        )
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"{x['remarks']}, is date key"
            if x["var"] in self.dkeys
            else x["remarks"],
            axis=1,
        )
        feature_selection_report["remarks"] = feature_selection_report.apply(
            lambda x: f"x['remarks], is label"
            if x["var"] == self.label
            else x["remarks"],
            axis=1,
        )
        feature_selection_report = feature_selection_report.sort_values(
            "gain_importance", asending=False
        )
        feature_selection_report = feature_selection_report[
            [
                "var",
                "used_as_model_input",
                "is_model_selected_flag",
                "remarks",
                "gain_importance",
                "split_importance",
            ]
        ].fillna("NA:dropped_from_model_input")
    ...
    def main(self):
        ...
        writer = ExcelWriter(
            base_folder="data/samples/", filename="MVP2_demo_48_intents"
        )
    ...
    def label_encode_train(self, train, label):
        ...
        label_encoder_artefacts = {
            "label_encoders": {"lab_to_idx": lab_to_idx, "idx_to_lab": idx_to_lab}
        }
    ...
    def label_encode_score(self, score, label):
        ...
        lab_to_idx = self.artefact_handler.get_artefact("training")["label_encoders"][
            "lab_to_idx"
        ]
        score[label] = score[label].map(lab_to_idx)
        if score[label].isnull().sum() > 0:
            labels_not_found = [
                lab for lab in y_score.unique() if lab not in lab_to_idx.keys()
            ]
    ...
    def train_test_split(self, df_pd):
        ...
        X_train_cin, X_test_cin = train_test_split(
            X_cin, test_size=float(0.2), random_state=int(99)
        )
    ...
    def get_train_test_oos_for_training(self, df_pd):
        ...
        df_pd_train_test = df_pd[
            df_pd["date"].apply(lambda x: (x >= train_start) and (x < train_end))
        ]
        train, test = self.train_test_split(df_pd_train_test)

        # get OOS dataset
        oos_df = df_pd[
            df_pd["date"].apply(lambda x: (x >= test_start) & (x <= test_end))
        ]
    ...
    def encode_categorical_train(self, df_pd, categorical_columns):
        ...
        self.artefact_handler.update_dict_artefact(
            name="training",
            key="categorical_feature_maps",
            value=categorical_feature_maps,
        )
        return df_pd

    def encode_categorical_score(self, df_pd, categorical_columns):
        df_pd = df_pd.copy()
        categorical_feature_maps = self.artefact_handler.get_artefact("training")[
            "categorical_feature_maps"
        ]
        for cat in tqdm.tqdm(categorical_columns):
    ...
    def encode_categorical_retrain_score(self, df_pd, categorical_columns):
        df_pd = df_pd.copy()
        categorical_feature_maps = self.artefact_handler.get_artefact("retraining")[
            "categorical_feature_maps"
        ]
        for cat in tqdm.tqdm(categorical_columns):
            cat_map = categorical_feature_maps[cat]
